...
REQUIRED_HISTORY_RECORD_FIELDS = {
    ...
    "error_type",
    "error_message",
    "feedback_export_records_path",
    "feedback_manifest_path",
    "phase14_feedback_acceptance_report_path",
    "phase15_feedback_daily_summary_path",
    "records_count",
    "created_at",
}
...
def _normalize_row(row: Any) -> Dict[str, Any]:
    ...
    raise TypeError(f"unsupported phase17 history row type: {type(row)!r}")
...
def validate_phase17_feedback_history_payloads(*, history_payload: dict, latest_payload: dict) -> dict:
    ...
    if not records:
        raise RuntimeError("feedback history records empty")
    ...
    if "records" in latest:
        raise RuntimeError("latest payload must not contain records")
