...
DEFAULT_REPORT_OUTPUT_NAME = "phase17_feedback_history_acceptance_report.json"
...
def _manual_check_points(stage: str) -> list[str]:
    if stage == "success":
        return [
            "check phase16 history remains append-only",
            "check latest points to the final history record",
            "spot-check required history fields remain present",
        ]
    if stage == "phase16_history_load":
        return [
            "check phase16_feedback_daily_history.json exists",
            "check the file is readable JSON",
            "check the path still points inside data/",
        ]
    if stage == "phase16_latest_load":
        return [
            "check phase16_feedback_daily_latest.json exists",
            "check the file is readable JSON",
            "check the path still points inside data/",
        ]
    return [
...
def _build_failure_report(
    *,
    ...
    return {
        ...
        "phase16_feedback_daily_history_path": str(history_path.resolve()),
        "phase16_feedback_daily_latest_path": str(latest_path.resolve()),
        "records_count": None,
        "latest_run_id": None,
...
def run_phase17_feedback_history_acceptance(data_dir="data") -> dict:
    ...
    try:
        history_payload = _load_json_object(history_path, missing_label="phase16 feedback history")
    except Exception as error:
        _write_report(
            _build_failure_report(
                failed_stage="phase16_history_load",
                error=error,
    ...
    try:
        latest_payload = _load_json_object(latest_path, missing_label="phase16 feedback latest")
    except Exception as error:
        _write_report(
            _build_failure_report(
                failed_stage="phase16_latest_load",
                error=error,
                history_path=history_path,
                latest_path=latest_path,
            ),
    ...
    try:
        validation = validate_phase17_feedback_history_payloads(
        ...
        _write_report(
            _build_failure_report(
                failed_stage="phase17_history_validator",
                error=error,
                history_path=history_path,
                latest_path=latest_path,
            ),
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the phase17 feedback history acceptance over frozen phase16 artifacts.")
...
if __name__ == "__main__":
    args = _parse_args()
    result = run_phase17_feedback_history_acceptance(data_dir=str(args.data_dir))
