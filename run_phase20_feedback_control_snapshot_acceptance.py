...
DEFAULT_REPORT_OUTPUT_NAME = "phase20_feedback_control_snapshot_acceptance_report.json"
...
def _load_json_object(path: Path) -> Dict[str, Any]:
    if not path.exists():
        raise FileNotFoundError(f"phase19 feedback control snapshot missing: {path}")
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise RuntimeError("phase19 feedback control snapshot payload must be an object")
    return dict(payload)
...
def _manual_check_points(stage: str) -> list[str]:
    if stage == "success":
        return [
            "check phase19 control snapshot remains compact and stable",
            "check required snapshot paths remain non-empty",
            "spot-check records_count and latest_run_id",
        ]
    if stage == "phase19_snapshot_load":
        return [
            "check phase19_feedback_control_snapshot.json exists",
...
def _build_failure_report(*, failed_stage: str, error: Exception, snapshot_path: Path) -> Dict[str, Any]:
    return {
        ...
        "phase19_feedback_control_snapshot_path": str(snapshot_path.resolve()),
        "records_count": None,
        "latest_run_id": None,
...
def run_phase20_feedback_control_snapshot_acceptance(data_dir="data") -> dict:
    ...
    try:
        ...
        _write_report(
            _build_failure_report(
                failed_stage="phase19_snapshot_load",
                error=error,
    ...
    try:
        validation = validate_phase20_feedback_control_snapshot_payload(snapshot_payload=snapshot_payload)
    except Exception as error:
        _write_report(
            _build_failure_report(
                failed_stage="phase20_snapshot_validator",
                error=error,
                snapshot_path=snapshot_path,
            ),
            report_path=report_path,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the phase20 feedback control snapshot acceptance over frozen phase19 artifacts.")
...
if __name__ == "__main__":
    args = _parse_args()
    result = run_phase20_feedback_control_snapshot_acceptance(data_dir=str(args.data_dir))
